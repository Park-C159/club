...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(args.epochs):
            pbar = ProgressBar(n_total=len(train_dataloader), desc=f'Epoch:{i+1}')
...
class EETrainer(Trainer):
    ...
    def training_step(self, model, item):
        ...
        labels = item[3].to(self.args.device).long()
...
class ERTrainer(Trainer):
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            # sub_start_preds.extend((sub_start_logits.detach().view(-1) >= 0.5).long()[active_index])
            # sub_end_preds.extend((sub_end_logits.detach().view(-1) >= 0.5).long()[active_index])
            # obj_start_preds.extend((obj_start_logits.detach().view(-1) >= 0.5).long()[active_index])
            # obj_end_preds.extend((obj_end_logits.detach().view(-1) >= 0.5)[active_index])
            #
            # sub_start_trues.extend(sub_start_label.detach().view(-1)[active_index].tolist())
            # sub_end_trues.extend(sub_end_label.detach().view(-1)[active_index].tolist())
            # obj_start_trues.extend(obj_start_label.detach().view(-1)[active_index].tolist())
            # obj_end_trues.extend(obj_end_label.detach().view(-1)[active_index].tolist())
